...
async def mysql_in_int(user_ids: str):
    response = await asyncio.to_thread(
        run_mysql,
        SQL=f"""SELECT user_id, username, firstname, lastname, email, role, age FROM Users WHERE IN({user_ids});""",
    )
...
async def mysql_in_string(usernames: str):
    response = await asyncio.to_thread(
        run_mysql,
        SQL=f"""SELECT user_id, username, firstname, lastname, email, role, age FROM Users WHERE IN({usernames});""",
    )
